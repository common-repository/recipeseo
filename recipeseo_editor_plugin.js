(function() {
	...
	tinymce.create('tinymce.plugins.amdEditRecipeSeo', {
        ...
        init : function(ed, url) {
            ...
            ed.addCommand('WP_EditRecipe', function() {
                var el = ed.selection.getNode(), vp = tinymce.DOM.getViewPort(), H = vp.h, W = ( 720 < vp.w ) ? 720 : vp.w, cls = ed.dom.getAttrib(el, 'class'), id = ed.dom.getAttrib(el, 'id').replace('amd-recipeseo-recipe-', '');
            ...
            ed.onMouseDown.add(function(ed, e) {
                ...
                if ( e.target.nodeName == 'IMG' && ed.dom.getAttrib(e.target, 'class').indexOf('mceItem') == -1 && ed.dom.getAttrib(e.target, 'class').indexOf('amd-recipeseo-recipe') === 0) {
        ...
        _do_shcode : function(co) {
            return co.replace(/\[amd-recipeseo-recipe:([0-9]+)\]/g, function(a, b) {
                return '<img class="amd-recipeseo-recipe" id="amd-recipeseo-recipe-'+b+'" alt="" src="' + baseurl + '/wp-content/plugins/' + dirname + '/recipeseo-placeholder.png" />';
            });
        },

        _get_shcode : function(co) {
            return co.replace(/\<img class="amd-recipeseo-recipe" id="amd-recipeseo-recipe-([0-9]+).*?\>/g, function(a, b){
                
                return '[amd-recipeseo-recipe:'+b+']';
            });
        },
        ...
        getInfo : function() {
            return {
                longname : "RecipeSEO Plugin",
                author : 'Allison Day',
                authorurl : 'http://recipeseo.com',
                infourl : 'http://recipeseo.com',
                version : "1.3"
            };
        }
    });

    tinymce.PluginManager.add('amdrecipeseo', tinymce.plugins.amdEditRecipeSeo);
})();